...
const SCHEDULE = [
  {
    ...
    events: [
      ...
      { time: '08:00 PM', title: 'EDM Night: DJ Concert', venue: 'Main Stage', icon: <PartyPopper className="w-4 h-4" />, color: 'var(--neon-lime)' },
    ]
  }
];
                ...
                <div className="flex flex-col gap-8 sm:gap-10">
                  {dayData.events.map((event, eventIdx) => (
                    <div key={eventIdx} className="relative flex items-center w-full group">
                      {/* Center Node / Dot */}
                      <div className="absolute left-[30px] sm:left-[60px] -translate-x-1/2 w-4 h-4 rounded-full border-2 border-[#030014] bg-white transition-all duration-500 scale-125 sm:scale-100 sm:group-hover:scale-150 shadow-[0_0_10px_white] z-20" />
                      ...
                      >
                        ...
                        >
                          ...
                          <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 transition-colors duration-300">
                            {event.title}
                          </h3>
